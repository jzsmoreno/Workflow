...
class SimpleNetRNN(tf.keras.models.Model):
    def __init__(self, n_neurons):
        ...
        self.network = tf.keras.Sequential([
            tf.keras.layers.SimpleRNN(self.n_neurons, return_sequences=True
            , input_shape = [None, 1]),
...
class SimpleNetGRU(tf.keras.models.Model):
    def __init__(self, filters, kernel_size, n_units, strides):
        ...
        self.network = tf.keras.Sequential([
            ...
            strides=self.strides, padding='valid', input_shape=[None, 1]),
...
if __name__ == "__main__":
    ...
    history = model.fit(x_train[:, :, np.newaxis], y_train, epochs = 30
