...
class MiniSom(object):
    def __init__(
        self, x, y, input_len, sigma=1.0, learning_rate=0.5, decay_function=None, random_seed=None
    ):
        """
        ...
        for i in range(x):
            for j in range(y):
                self.weights[i, j] = self.weights[i, j] / fast_norm(
                    self.weights[i, j]
                )  # normalization
    ...
    def update(self, x, win, t):
        ...
        while not it.finished:
            ...
            # normalization
            self.weights[it.multi_index] = self.weights[it.multi_index] / fast_norm(
    ...
    def _init_T(self, num_iteration):
        ...
        self.T = (
            num_iteration / 2
        )  # keeps the learning rate nearly constant for the last half of the iterations
        ...
        while not it.finished:
            for ii in range(it.multi_index[0] - 1, it.multi_index[0] + 2):
                for jj in range(it.multi_index[1] - 1, it.multi_index[1] + 2):
                    if (
                        ii >= 0
                        and ii < self.weights.shape[0]
                        and jj >= 0
                        and jj < self.weights.shape[1]
                    ):
...
class TestMinisom:
    def setup_method(self, method):
        ...
        for i in range(5):
            for j in range(5):
                assert_almost_equal(
                    1.0, linalg.norm(self.som.weights[i, j])
    ...
    def test_decay_function(self):
        assert self.som._decay_function(1.0, 2.0, 3.0) == 1.0 / (1.0 + 2.0 / 3.0)
